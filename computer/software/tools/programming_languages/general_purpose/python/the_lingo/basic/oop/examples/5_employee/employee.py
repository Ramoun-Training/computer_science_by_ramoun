...
class Employee:
    ...
    def full_name(self):
        """
        return to the user fullname
        """
        return f"{self.first} {self.last}"

    def apply_raise(self):
        '''add raise to an individual employee'''
        # self.pay = int(self.pay * Employee.raise_amount)
        self.pay = int(
            self.pay * self.raise_amount
        )  # using self.raise_amount instead of Employee.raise_amount
        # will allow us to override the class attribute freely for each instance as we want

    # creating a class method for cases where we want something to be the same for are instances
    @classmethod
    def change_raise_amount(cls, amount):
        '''change raise amount for the class'''
        cls.raise_amount = amount

    # creating an alternative contractor
    @classmethod
    def from_string(cls, emp_str):  # using 'from_' is a convention
        """construct an Employee object from a string"""
        first, last, pay = emp_str.split("-")
        return cls(first, last, pay)

    @staticmethod
    def is_workday(the_day):
        '''check if it is a working day or a day-off'''
        if the_day.weekday() == 5 or the_day.weekday() == 6:
            return False
...
# using change_raise_amount with an object
...
persons = [Person("ramoun"), Person("python")]
